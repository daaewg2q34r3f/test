...
// 直连 fetch，绕过系统代理（VPN 等），超时放宽到 5 分钟兼容大模型慢启动
const directFetch = ((url: any, init?: any) =>
  undiciFetch(url, {
    ...init,
    dispatcher: new Agent({ headersTimeout: 300_000, bodyTimeout: 300_000 }),
...
const buildOptions = async (input: AIInput<any>, config: AIConfig) => {
  let sqlTextModelConfig: Partial<{ model: string; apiKey: string; baseURL: string }> = {};
  try {
    sqlTextModelConfig = await u.getConfig("text");
  } catch {
    // 数据库无配置时，直接使用传入的 config
  }
  ...
  const owned = modelList.find((m) => m.model === model) ?? {
    ...
    model: model ?? "",
    responseFormat: "object" as const,
    image: false,
    think: false,
    tool: true,
  ...
  const modelInstance = owned.instance({ apiKey, baseURL, fetch: directFetch });
  ...
  return {
    config: {
      model: ((modelInstance as any).chat ?? modelInstance)(model) as LanguageModel,
...
ai.invoke = async (input: AIInput<any>, config: AIConfig = {}) => {
  ...
  await logAIInput({
    stage: input.log?.stage || "text",
    action: input.log?.action || "text_invoke",
    targetId: input.log?.targetId,
    model: options.modelName,
    manufacturer: options.manufacturer,
    systemPrompt: input.system,
    prompt: input.prompt,
    messages: input.messages,
    images: input.log?.images,
    extra: {
      ...input.log?.extra,
      hasTools: Boolean(input.tools),
      hasStructuredOutput: Boolean(input.output),
...
ai.stream = async (input: AIInput, config: AIConfig = {}) => {
  ...
  await logAIInput({
    stage: input.log?.stage || "text",
    action: input.log?.action || "text_stream",
    targetId: input.log?.targetId,
    model: options.modelName,
    manufacturer: options.manufacturer,
    systemPrompt: input.system,
