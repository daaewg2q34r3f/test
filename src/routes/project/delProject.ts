...
export default router.post(
  ...
  async (req, res) => {
    ...
    const workflowIds = await u.db("t_workflow").where("projectId", id).select("id");
    const workflowIdList = workflowIds.map((item: any) => item.id).filter(Boolean);
    await u.db("t_workflow").where("projectId", id).delete();
    if (workflowIdList.length > 0) {
      await u.db("t_workflow_run").whereIn("workflowId", workflowIdList).delete();
    }
    await u.db("t_workflow_run").where("projectId", id).delete();
