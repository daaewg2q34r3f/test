...
router.ws("/", async (ws, req) => {
  ...
  try {
    config = await u.getConfig("language");
  } catch (e: any) {
    ws.send(JSON.stringify({ type: "error", data: e?.message ?? "语言模型未配置，请在设置中配置语言生成模型" }));
    ws.close(1000, "未配置语言模型");
    return;
  }
  ...
  // 一键生成：进度事件
  agent.emitter.on("generateAllStart", () => {
    ws.send(JSON.stringify({ type: "generateAllStart" }));
  });
  agent.emitter.on("generateAllProgress", (data) => {
    ws.send(JSON.stringify({ type: "generateAllProgress", data }));
  });
  agent.emitter.on("generateAllComplete", (data) => {
    ws.send(JSON.stringify({ type: "generateAllComplete", data }));
  });
  agent.emitter.on("generateAllError", (data) => {
    ws.send(JSON.stringify({ type: "generateAllError", data }));
  });

  // 发送初始化完成消息，通知前端可以开始发送消息
  ws.send(JSON.stringify({ type: "init", data: { projectId, scriptId } }));

  type DataTyype = "msg" | "cleanHistory" | "generateShotImage" | "replaceShot" | "generateAll";
    ...
    try {
      switch (data?.type) {
        ...
        case "replaceShot":
          ...
          break;
        case "generateAll":
          // Fire-and-forget: progress is streamed via emitter events
          agent.generateAll().catch((err: any) => {
            ws.send(JSON.stringify({ type: "generateAllError", data: { message: err?.message || "生成失败" } }));
          });
          break;
        default:
          break;
