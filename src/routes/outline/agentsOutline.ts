...
router.ws("/", async (ws, req) => {
  ...
  agent.emitter.on("data", (text) => {
    if (streamChunkCount === 0) {
      ...
      console.log(`[outline stream] 首个token到达，延迟: ${firstChunkTime - reqStartTime}ms`);
    }
    streamChunkCount++;
  ...
  agent.emitter.on("response", async (text) => {
    ...
    console.log(`[outline stream] 响应完成 总耗时:${totalTime}ms 流式块数:${streamChunkCount} 响应长度:${text?.length ?? 0}字`);
